...
int create_and_bind(const char *port) {
    ...
    if (s != 0) {
        logln_info("getaddrinfo: %s", gai_strerror(s));
        return -1;
    }
...
//    NSLog(@"server_recv_cb %d", server?server->stage:-1);
    ...
    if (server->stage == 5) {
        ...
        if (request->cmd != SOCKS_CMD_CONNECT) {
            logln_info("unsupported cmd: %d", request->cmd);
...
//#if !TARGET_OS_IPHONE
            logln_info("Connecting an IPv4 address, please configure your browser to use hostname instead: https://github.com/clowwindy/gsp/wiki/Troubleshooting");
//#endif
...
//#if !TARGET_OS_IPHONE
            ...
            temp[name_len] = '\0';
           logln_info("Connecting %s", temp);
//#endif
        } else {
           logln_info("unsupported addrtype: %d", request->atyp);
            close_and_free_server(EV_A_ server);
            close_and_free_remote(EV_A_ remote);
            return;
        }
        ...
        if (n != addr_len) {
            logln_info("header not completely sent: n != addr_len: n==%d, addr_len==%d", n, (int)addr_len);
            close_and_free_remote(EV_A_ remote);
            close_and_free_server(EV_A_ server);
            return;
        }
        ...
        *((unsigned short *)(server->buf + 4 + sizeof(struct in_addr)))
        ...
        if (r < reply_size) {
            logln_info("header not complete sent");
            close_and_free_remote(EV_A_ remote);
            close_and_free_server(EV_A_ server);
            return;
        }
...
void set_config(const char *server, const char *remote_port, const char* password, const char* method) {
    ...
    logln_info("calculating ciphers");
...
int local_main ()
{
    ...
    if (listenfd < 0) {
        logln_error("bind() error..");
        return 1;
    }
    if (listen(listenfd, SOMAXCONN) == -1) {
        logln_error("listen() error.");
        return 1;
    }
   logln_info("server listening at port %s", "1080");
